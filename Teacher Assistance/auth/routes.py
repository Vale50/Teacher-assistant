...
auth_bp = Blueprint('auth', __name__, url_prefix='/api/student/profile')
...
@auth_bp.route('/<int:student_id>', methods=['GET'])
@jwt_required()
        ...
        if 'files' in request.files:
            ...
            for file in files:
                ...
                if file_ext in s3_service.ALLOWED_VIDEO_EXTENSIONS:
                    ...
                    file_type = 'image'
                elif file_ext in s3_service.ALLOWED_DOCUMENT_EXTENSIONS:
                    file_type = 'document'
                else:
                    file_type = 'avatar'
...
@auth_bp.route('/posts/my-posts', methods=['GET'])
@jwt_required()
def get_my_posts():
...
@auth_bp.route('/posts/student/<int:student_id>', methods=['GET'])
@jwt_required()
...
@auth_bp.route('/timeline', methods=['GET'])
@jwt_required()
def get_timeline():
...
@auth_bp.route('/posts/<int:post_id>/unlike', methods=['POST'])
@jwt_required()
...
@auth_bp.route('/posts/<int:post_id>/likes', methods=['GET'])
@jwt_required()
...
@auth_bp.route('/comments/<int:comment_id>', methods=['PUT'])
@jwt_required()
